...
| Metric | Value |
Our core objective is to assist specialists in gastrointestinal image interpretation by delivering accurate predictions with transparent visual evidence through Grad-CAM, while staying practical under real memory constraints.
| **Total Labeled Images** | 10,662 |
| **Number of Classes** | 23 |
| **Image Resolution** | Variable (resized to 384×384) |
| **Split Strategy** | Stratified random split |
Endoscopy generates large volumes of image data. Manual review is time-intensive and can be affected by inter-observer variability, especially in subtle or early-stage abnormalities.

At the same time, automated GI classification is technically difficult because of severe class imbalance, subtle inter-class visual differences, and high computational cost for high-resolution medical images.
...
- Gradient accumulation (effective batch size 16 from physical batch size 2)
| Category | Conditions |
|----------|------------|
...
Key training setup points for the committee:
- 25 epochs per model
- Optimizer: AdamW, learning rate 1e-5, weight decay 0.01
- Warmup: 5 epochs, followed by cosine decay
...
---
- DeiT3 Small accuracy: 92.56 percent
- ViT Base accuracy: 93.12 percent
- Ensemble plus TTA accuracy: 93.56 percent

Observed gains from ensembling:
- +1.00 percent over DeiT3 Small
- +0.44 percent over ViT Base
> **SPEAK:**
Additional weighted metrics reported:
- Ensemble precision: 92.56 percent
- Ensemble recall: 93.56 percent
- Ensemble F1-score: 92.89 percent

Notable per-class outcomes include perfect classification for five classes:
- bbps-0-1
- bbps-2-3
- cecum
- pylorus
...
Comparative benchmark statement:
Across a comparison against 14 state-of-the-art methods, this framework is positioned as a top-performing and computationally practical approach for full 23-class HyperKvasir classification.
...
Computational practicality statement:
- Training memory: approximately 4 to 6 GB (model-dependent)
- Inference memory: approximately 0.5 GB
- Training duration: around 2 to 3 hours per model for 25 epochs
| Input Resolution | 384×384 RGB |
| Pre-training | ImageNet-22k |
Answer: They have complementary strengths. DeiT3 contributes training efficiency and ViT contributes representational capacity. Weighted ensemble with TTA improved results to 93.56 percent compared with 92.56 percent and 93.12 percent for the individual models.
...
Answer: We used focal loss, label smoothing, stratified splitting, and MixUp, then evaluated with weighted precision, recall, and F1 to reflect class imbalance more reliably.
### Model 2: ViT Base Patch16 384

Answer: External multi-center validation, stronger rare-class handling through few-shot or meta-learning, calibration and uncertainty estimation, role-based access control, and audit-compliant logging.
|---------------|-------|
| Architecture | Standard Vision Transformer (Base) |
| Parameters | ~86 million |
We built a full-stack GI endoscopy diagnostic platform that combines DeiT3 Small and ViT Base transformer models with Grad-CAM explainability. In notebook-based training on Hyper-Kvasir, we process 10,662 images across 23 classes using stratified 70/15/15 splits and advanced techniques such as mixed precision, gradient accumulation, focal loss, label smoothing, MixUp, and TTA. The weighted ensemble reaches 93.56 percent accuracy, improving over standalone DeiT3 at 92.56 percent and ViT Base at 93.12 percent, while keeping inference memory near 0.5 GB. We then deploy traced TorchScript models through a FastAPI backend and a React frontend, fully containerized with Docker Compose. This creates an end-to-end, transparent, and clinically practical decision-support system ready for external validation and pilot deployment.
| Input Resolution | 384×384 RGB |
